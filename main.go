...
func sendFile(ctx context.Context, s3ndUrl, uri url.URL, file string) (*upload.RequestStatus, error) {
	// http.PostForm is not used here because it does accept a context
	req, err := http.NewRequestWithContext(ctx, "POST", s3ndUrl.String(), strings.NewReader(url.Values{
		"file": {file},
		"uri":  {uri.String()},
	}.Encode()))
	if err != nil {
...
func sendFiles(s3ndUrl url.URL, files map[string]url.URL) (*[]*fileStatus, error) {
	...
	for f, uri := range files {
		...
		go func() {
			defer wg.Done()
			status, err := sendFile(context.TODO(), s3ndUrl, uri, f)
			statusCh <- &fileStatus{
				s3ndUrl:       s3ndUrl,
				uri:           uri,
				file:          f,
...
// Submit all files for a given host to the s3nd service.
func hostWorker(h *hostWorkerInput) error {
	...
	if *h.conf.SendFirst != "" {
		...
		// XXX
		startSendFiles := time.Now()
		statuses, err := sendFiles(s3ndUrl, files)
		sendFilesDuration := time.Since(startSendFiles)
		if err != nil {
			logger.Error(
				fmt.Sprintf("%v failed", *h.conf.SendFirst),
				"host", h.host,
				"duration_seconds", time.Since(h.start).Seconds(),
				"took_seconds", sendFilesDuration.Seconds(),
				"run", h.run,
				"error", err,
			)
			return err
		...
		logger.Info(
			fmt.Sprintf("files == %v done", *h.conf.SendFirst),
			"host", h.host,
			"duration_seconds", time.Since(h.start).Seconds(),
			"run", h.run,
			"total_size_bytes", totalBytes,
			"files_count", len(*statuses),
			"transfer_rate_mbits", (float64(totalBytes*8)/sendFilesDuration.Seconds())/(1<<20),
		)
	}
	...
	var msg string
	if *h.conf.SendFirst != "" {
		msg = fmt.Sprintf("files != %v done", *h.conf.SendFirst)
	} else {
		msg = "all files sent"
	}
